...
5. Paste the certificate.pem that was generated by the script in the previous step (starts with `-----BEGIN CERTIFICATE-----` and ends with `-----END CERTIFICATE-----`), then click `Create new key` and `Save` in the keyset dialog.
