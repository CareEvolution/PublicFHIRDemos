...
(function() {
	...
	SmartApps.factory("authenticator", ["$http", "urlParameters", function($http, urlParameters) {
		...
		return {
			login: function(clientID, onSuccess, onError, scope, clientSecret) {
				...
				if (fhirUrl) {
					patientID = urlParameters["patientID"];
						...
						if (!fhirUrl) {
							...
							if (!code) {
								...
								if (!clientSecret) {
									...
									var basicCredentials = encodeURIComponent(clientID) + ":" + encodeURIComponent(clientSecret);
